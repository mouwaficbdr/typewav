...
import { getChordAtIndex, getPentatonicNote } from '@typewav/audio-engine';
...
export function useAudioEngine() {
  ...
  const playNote = useCallback(
    async (char: string, wordIndex: number) => {
      ...
      const noteToPlay = chord.notes.includes(basePitch) ? baseNote : baseNote; // Fallback : on joue la note pentatonique directement
  ...
  const triggerResume = useCallback(async () => {
    ...
    const Tone = await import('tone');
    const reverb = reverbRef.current as {
      wet: {
